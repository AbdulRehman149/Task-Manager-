...
exports.getTasks = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ tasks, status: true, msg: 'Tasks search successful' });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
...
exports.getTask = async (req, res) => {
  try {
    ...
    res.status(200).json({ task, status: true, msg: 'Task search successful' });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
  ...
  try {
    ...
    if (task.user != req.user.id) {
      return res
        ...
        .json({
          status: false,
          msg: 'Forbidden! Task of another user cannot be updated'
        });
    }
    ...
    res.status(200).json({ task, status: true, msg: 'Task update successful' });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
